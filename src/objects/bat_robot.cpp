...
bool TA_BatRobot::update() {
    switch(state) {
        ...
        case STATE_ACTIVE: {
            ...
            float centeredX = position.x + 12;
            float deltaY = characterPosition.y - position.y - 8;
...
float TA_BatRobot::findOptimalSpeed(float deltaY) {
    auto distance = [&](float jumpSpeed) {
        float t = -(jumpSpeed - gravity / 2) / gravity;
        return t * jumpSpeed + t * (t - 1) / 2 * gravity;
    };
    float left = 1 * speed, right = 3.2 * speed, eps = 1e-3;
